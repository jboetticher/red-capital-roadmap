...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(graph.shortestPath("D", "B"));
    System.out.println(graph.getPathCost("D", "B"));
    
    try {
      graph.shortestPath(null, "A");
    ...
    protected Path dijkstrasShortestPath(T start, T end) {   
      ...
      // What if the either are null?
      if (start == null || end == null) throw new NoSuchElementException();
      // What if they are the same node?
      if (start.equals(end))
        return new Path(vertices.get(start));
        //return new Path(new Path(vertices.get(start)), new Edge(vertices.get(end), 0));
      ...
      while(!pq.isEmpty()) {
        //System.out.println(count1); count1++; count2 = 0;
        
        // Mark path end node as visited
        Path current = pq.remove();
        visited.remove(current.end.data);
        visited.put(current.end.data, true);
        //System.out.println("Visited " + current.end.data);
        List<Edge> edges = current.end.edgesLeaving;
        
        for (Edge e : edges) {
          //System.out.println(" - " + count2); count2++;
          Path newPath = new Path(current, e);
          
          // Check if node is visited
          if (visited.containsKey(e.target.data)) {
            if (!visited.get(e.target.data)) {
              // Node has been seen but not visited
              pq.add(newPath);
              //System.out.println("Edge to " + newPath.end);
            }
            // else node has been visited
          } else {
            // Node has not been seen nor visited
            pq.add(newPath);
            //System.out.println("- Edge to " + newPath.end.data);
          }
          
          // Check if we found the shortest path to the end node
          if (e.target.data.equals(end)) { 
            if (shortestPath == null) shortestPath = newPath;
            else if (newPath.compareTo(shortestPath) < 0) shortestPath = newPath;
          }
        }
      }
      
      if (shortestPath == null) throw new NoSuchElementException();
